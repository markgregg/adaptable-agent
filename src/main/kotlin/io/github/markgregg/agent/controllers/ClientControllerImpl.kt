...
class ClientControllerImpl(
    ...
    constructor(serviceConfiguration: io.github.markgregg.agent.configuration.ServiceConfiguration) : this( serviceConfiguration, { PublishSubject.create() }, { PublishSubject.create() })
    companion object {
    ...
    override fun processMessage(session: WebSocketSession, message: TextMessage) {
        try {
            when {
                message.payload.contains("io.github.markgregg.common.protocol.StartTest") -> addStartTestMessage(session, message.payload)
                message.payload.contains("io.github.markgregg.common.protocol.EndTest") -> processEndTestMessage(session, message.payload)
                message.payload.contains("io.github.markgregg.common.protocol.MakeAvailable") -> processMakeAvailableMessage(session, message.payload)
                message.payload.contains("io.github.markgregg.common.protocol.MakeUnavailable") -> processMakeUnavailableMessage(session, message.payload)
                message.payload.contains("io.github.markgregg.common.protocol.Message") -> processResponseMessage(session, message.payload)
                message.payload.contains("io.github.markgregg.common.protocol.EndpointResponse") -> processEndpointResponse(message.payload)
