...
class ServiceConfigurationImplTest : FunSpec() {
	init {
		...
		test("keyStoreType") {
			val result = ServiceConfigurationImpl(ServiceConfigLoader.load(xmlProps.byteInputStream()), typeDiscoveryMock())
		...
		test("endPoints") {
			...
			val result = ServiceConfigurationImpl(ServiceConfigLoader.load(xml.byteInputStream()), typeDiscovery)
		...
		test("load with bad endpoint") {
			val exception = shouldThrowExactly<EndPointDefinitionException> {
				ServiceConfigurationImpl(ServiceConfigLoader.load(xmlBadEndPoint.byteInputStream()), typeDiscoveryMock())
			}
		...
		test("load with no matching constructor") {
			val exception = shouldThrowExactly<EndPointDefinitionException> {
				...
				ServiceConfigurationImpl(ServiceConfigLoader.load(xmlNoMatchingConstructor.byteInputStream()), typeDiscoveryMock())
			}
		...
		test("load with duplicate endpoint") {
			val exception = shouldThrowExactly<DuplicateEndPointException> {
				ServiceConfigurationImpl(ServiceConfigLoader.load(xmlDuplicateEndPoint.byteInputStream()), typeDiscoveryMock())
			}
